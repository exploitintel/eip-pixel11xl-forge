...
scan_exact_processes() {
  ...
  SCAN_COUNT=0
  SCAN_PID=
  for SCAN_DIRECTORY in "$PROC_ROOT"/[0-9]*; do
    [ -d "$SCAN_DIRECTORY" ] || continue
    SCAN_CANDIDATE=${SCAN_DIRECTORY##*/}
    # The shell's file-identity test avoids spawning one readlink process for
    # every Android PID while retaining an exact executable inode check.
    [ "$SCAN_DIRECTORY/exe" -ef "$SCAN_EXECUTABLE" ] 2>/dev/null || continue
...
require_start_prerequisites() {
  ...
  if ! grep -q " $DOCKER_DATA ext4 " "$MOUNTS"; then
    ...
    mount -t ext4 -o noatime,nodev "$DISK_LOOP" "$DOCKER_DATA" || die 'cannot mount Docker disk'
  fi
  echo 1 > "$IP_FORWARD"
  ...
  [ -f "$DOCKER_DISK" ] && [ ! -L "$DOCKER_DISK" ] ||
    die 'existing Docker data image is unavailable; refusing to create a replacement'
  awk -v target="$DOCKER_DATA" '$2 == target && $3 == "ext4" { found=1 } END { exit found ? 0 : 1 }' "$MOUNTS" 2>/dev/null ||
    die 'Docker data image is not mounted; reboot with the existing host initialization enabled'
}
...
stop_exact_daemon() {
  STOP_PID=$(cat "$DOCKER_PIDFILE" 2>/dev/null) || die 'Docker pid is unavailable'
  ...
  ! docker_info || die 'Docker did not stop after TERM'
  STOP_ATTEMPT=0
  while [ -d "$PROC_ROOT/$STOP_PID" ] && [ "$STOP_ATTEMPT" -lt "$STOP_TRIES" ]; do
    sleep "$SLEEP_SECONDS"
    STOP_ATTEMPT=$((STOP_ATTEMPT + 1))
  done
  [ ! -d "$PROC_ROOT/$STOP_PID" ] || die 'Docker daemon did not exit after TERM'
  rm -f "$DOCKER_PIDFILE" "$DOCKER_SOCKET"
}
