...
while IFS= read -r entry; do
  case "$entry" in
    image-kodiak-*.zip|*/image-kodiak-*.zip)
      [[ -z "$NESTED_IMAGE" ]] || die 'factory ZIP contains more than one kodiak image archive'
      NESTED_IMAGE=$entry
      ;;
  esac
done < <(unzip -Z1 "$FACTORY_ZIP")
[[ -n "$NESTED_IMAGE" ]] || die 'this is not the Pixel 11 Pro XL kodiak factory ZIP'

printf 'Extracting Google build CD1A.260714.001.A9...\n'
unzip -p "$FACTORY_ZIP" "$NESTED_IMAGE" >"$WORK/image-kodiak.zip"
unzip -p "$WORK/image-kodiak.zip" boot.img >"$WORK/boot.img"
unzip -p "$WORK/image-kodiak.zip" init_boot.img >"$WORK/init_boot.img"

hash_file "$WORK/boot.img"
[[ "$HASH" == 5fc827ab5adfaf81f84cd7b1ab8675684e5588aaff832e9ba45051a72d0b06a2 ]] || \
  die 'boot.img does not match Google build CD1A.260714.001.A9'
hash_file "$WORK/init_boot.img"
[[ "$HASH" == b0f444b06ca749a51452a1efb84b78d99d5a72e8026f8f6b151ed7d0a8f458d5 ]] || \
  die 'init_boot.img does not match Google build CD1A.260714.001.A9'
...
"$ADB_BIN" -s "$SERIAL" shell "chmod 700 $REMOTE_KSUD && $REMOTE_KSUD boot-patch --boot $REMOTE_INIT --kmi android16-6.12 --out /data/local/tmp --out-name eip-prepare-ksu-init-boot.img" >/dev/null
"$ADB_BIN" -s "$SERIAL" pull "$REMOTE_OUTPUT" "$WORK/ksu-init-boot.img" >/dev/null

hash_file "$WORK/ksu-init-boot.img"
[[ "$HASH" == bd471feb086b8bd0466dd2c1ec52598fbaa4a97c4bde7d58f7b0f051f02e553f ]] || \
